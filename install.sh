...
function ToolsSettings({ isOpen, onClose }) {
  ...
  const saveSettings = async () => {
    ...
    try {
      ...
      if (kimiConfig.enabled && kimiConfig.apiKey) {
        // Save custom API configurations if enabled
        // Use user input directly as model name
        const modelPath = kimiConfig.model;
        ...
        const response = await fetch('/api/env-vars', {
          ...
          body: JSON.stringify({
            envVars: {
              ...
              ANTHROPIC_MODEL: modelPath
            }
          })
        });

        if (!response.ok) {
          throw new Error('Failed to save custom API configuration');
        }
      } else {
        ...
        if (!response.ok) {
          throw new Error('Failed to clear custom API configuration');
        }

        // Reset kimiConfig to default state after successful clearing
        setKimiConfig({
          enabled: false,
          apiKey: '',
          baseUrl: 'https://maas-api.lanyun.net/anthropic-k2/',
          model: 'k2'
        });
        ...
        if (data.envVars) {
          ...
          if (!originalEnvVars) {
            setOriginalEnvVars({
              ...
              ANTHROPIC_MODEL: data.envVars.ANTHROPIC_MODEL || null
            });
          }
          
          // Check if custom API is configured
          const isKimiConfigured = !!data.envVars.ANTHROPIC_BASE_URL;

          // Extract model from ANTHROPIC_MODEL env var
          let modelValue = 'k2'; // default
          if (data.envVars.ANTHROPIC_MODEL) {
            modelValue = data.envVars.ANTHROPIC_MODEL;
          }

          setKimiConfig({
            enabled: isKimiConfigured,
            apiKey: isKimiConfigured ? data.envVars.ANTHROPIC_API_KEY || '' : '',
            baseUrl: data.envVars.ANTHROPIC_BASE_URL || 'https://maas-api.lanyun.net/anthropic-k2/',
            model: modelValue
          });
        } else {
  ...
  const handleKimiRestore = async () => {
    ...
    try {
      ...
      if (response.ok) {
        // Reset custom API config to default state
        setKimiConfig({
          enabled: false,
          apiKey: '',
          baseUrl: 'https://maas-api.lanyun.net/anthropic/',
                ...
                <div className="space-y-4">
                        ...
                        <div className="space-y-4">
                          ...
                          <div>
                            ...
                            <Input
                              ...
                              placeholder="https://maas-api.lanyun.net/anthropic-k2/"
                              disabled={!kimiConfig.enabled}
                            />
                            <p className="text-xs text-muted-foreground mt-1">
                              Default:  https://maas-api.lanyun.net/anthropic-k2/
                            </p>
                          </div>
                          ...
                          <div>
                            ...
                            <Input
                              ...
                              onChange={(e) => setKimiConfig(prev => ({ ...prev, model: e.target.value }))}
                              placeholder="k2"
                              disabled={!kimiConfig.enabled}
                            />
                            <p className="text-xs text-muted-foreground mt-1">
                              Default: k2. You can enter any custom model name directly.
                            </p>
                          </div>
                        </div>
                      </div>
                  ...
                  <div className="bg-blue-50 dark:bg-blue-900/20 border border-blue-200 dark:border-blue-800 rounded-lg p-4">
                    ...
                    <ul className="text-sm text-blue-800 dark:text-blue-200 space-y-1">
                      ...
                      <li>• Environment variables ANTHROPIC_BASE_URL, ANTHROPIC_API_KEY, and ANTHROPIC_MODEL are set</li>
                      <li>• Model field: enter any model name directly (default: k2)</li>
